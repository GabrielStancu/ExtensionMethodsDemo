...
namespace ExtensionMethodsDemo
{
    class Program
    {
        ...
        private static async Task TestLightCapabilities(ILight light)
        {
            //Perform basic tests:
            light.SwitchOn();
            Console.WriteLine($"After switching the light on, the light is {(light.IsOn() ? "on" : "off")}.");
            light.SwitchOff();
            Console.WriteLine($"After switching the light off, the light is {(light.IsOn() ? "on" : "off")}.");
